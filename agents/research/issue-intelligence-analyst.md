...
获取的每份数据都会占用聚类和推理所需的上下文空间。获取最小字段，不要批量获取正文。
...
**硬性规则：**
- **每次获取使用一条 `gh` 命令** —— 使用 `--limit` 在单次调用中获取所有需要的 Issues。不要跨多次调用分页、通过 `tail`/`head` 管道或拆分获取。一次 `gh issue list --limit 200` 是可以的；两次调用分别获取 1-100 和 101-200 是不必要的。
