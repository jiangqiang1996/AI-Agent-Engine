...
置信度为 **高（0.80+）**：能引用标准文件中的具体规则并指向 diff 中违反它的具体行——规则和违规都是明确的。

置信度为 **中（0.60-0.79）**：规则存在于标准文件中但将其应用于此具体情况需要判断——例如，skill 描述是否充分"描述了做什么以及何时使用"，或文件是否小到可以符合 `@` 包含的资格。

置信度为 **低（0.60 以下）**：标准文件对这是否构成违规不明确，或规则可能不适用于此文件类型——抑制这些。
