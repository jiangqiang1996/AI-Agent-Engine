...
这是一条用户视角的一句话安装入口，风格上接近 `superpowers` 在 OpenCode 中的安装方式，也接近 `oh-my-openagent` README 里“复制给 AI 代理”的安装方式。

在 AI 代理执行安装前，它应先检查本地是否已经安装以下插件：
- `oh-my-openagent`
- `oh-my-opencode`
- `superpowers`

如果命中任意一个，代理应先在对话中给出明确警告：**这些插件可能与 AI Agent Engine 不兼容**，然后再继续安装或让用户确认是否继续。
...
```bash
curl -L https://gitee.com/jiangqiang1996/ai-agent-engine/raw/master/.opencode/INSTALL.md
```

执行安装前，先检查本地 opencode 配置与已安装 plugin 路径中是否存在 `oh-my-openagent`、`oh-my-opencode` 或 `superpowers`。若存在，先在对话中提示兼容性警告，再继续后续步骤。
...
3. 在你的 `opencode.json` 中加入 plugin 入口：
...
说明：
- `npm run build` 会同步生成 `.opencode/plugins/`、`.opencode/commands/`、`.opencode/agents/ae/` 里的运行时产物
- 如果你把仓库放在别的位置，请把上面的路径替换成你的实际安装路径
...
| 构建命令 | `npm run build` |
| 测试命令 | `npm run test` |
| opencode 配置 | `opencode.json` 中的 `plugin` 数组加载本地 plugin 入口 |
