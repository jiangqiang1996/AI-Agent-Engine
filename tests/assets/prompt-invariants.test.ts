...
const INVARIANTS: Invariant[] = [
  {
    ...
    mustNotContain: ['根据选择路由', '自适应粒度', 'module.md（§API + §Database）', '超限禁止压缩，按数字顺序分片'],
  },
  {
  ...
  },
  {
    ...
    mustContain: ['总览层按方向拆分为独立文件', '| database | `@database-designer` |', 'modules/<NN>-<m>/api.md', 'architecture.md'],
    mustNotContain: ['unified/split', '<ae-design路径>', '强制维度拆分', '无论文件大小，每个维度必须拆分为独立子文件', '自适应拆分', '压缩冗余内容', 'modules/<m>.md §API', '生成时拆分，非生成后拆分', '按数字顺序分片', '禁止压缩内容'],
  },
  {
    file: 'src/assets/skills/ae-design/SKILL.md',
  ...
  },
  {
    ...
    label: 'ae:prd 原型独立文件',
    mustContain: ['pages/', 'type: prd-page'],
    mustNotContain: [],
  },
]
