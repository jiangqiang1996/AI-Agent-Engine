...
const readme = readFileSync('docs/ae/solutions/README.md', 'utf8')
const patterns = readFileSync('docs/ae/solutions/patterns/critical-patterns.md', 'utf8')
...
describe('research-reviewer 经验库文本契约', () => {
  ...
  it('critical-patterns 应该只维护索引和关键模式', () => {
    expect(existsSync('docs/ae/solutions/patterns/critical-patterns.md')).toBe(true)
  ...
  it('research-reviewer 应该在经验库缺失时自然降级', () => {
    expect(reviewer).toContain('docs/ae/solutions/')
    expect(reviewer).toContain('docs/ae/solutions/patterns/critical-patterns.md')
