...
const REQUIRED_ENVIRONMENT_GATE_FILES = [
...
const ANTI_PATTERN_REGEXES = [
  /如已安装\s*agent-browser.{0,20}(直接|即可|继续|跳过|无需\s*环境)/,
  /CLI\s*可用时.{0,20}(跳过|无需|直接).{0,10}环境/,
  /agent-browser\s*(已安装|可用).{0,20}(无需|跳过|直接|即可).{0,10}(环境|proof)/,
...
const ENVIRONMENT_GATE_PATTERN = /ae:agent-browser|\/ae-agent-browser|ae-agent-browser-proof|环境门禁/
const EXECUTION_BAN_PATTERN =
  /未.*环境证明.*不得执行|未通过 proof 校验前不得执行|执行任何 `agent-browser` 命令前/
...
describe('agent-browser 环境门禁', () => {
  it('应该让所有已知浏览器消费方包含环境门禁语义', () => {
    for (const file of REQUIRED_ENVIRONMENT_GATE_FILES) {
      const content = readFileSync(file, 'utf8')

      expect(content, file).toMatch(ENVIRONMENT_GATE_PATTERN)
    }
  })

  it('浏览器消费方不应该保留手写 agent-browser 安装检查反模式', () => {
  ...
  it('包含可复制 agent-browser 命令的段落应该声明未完成环境证明前不得执行', () => {
    for (const file of scannedFiles()) {
      const content = readFileSync(file, 'utf8')
      ...
      for (const match of matches) {
        ...
        expect(nearby, `${file}:${lineNumber}`).toMatch(ENVIRONMENT_GATE_PATTERN)
        expect(nearby, `${file}:${lineNumber}`).toMatch(EXECUTION_BAN_PATTERN)
      }
    }
  })

  it('公开使用指南的截图代理流程应该声明环境证明前置', () => {
    ...
    for (const row of rows) {
      if (row.includes('@figma-design-sync') || row.includes('@design-iterator')) {
        expect(row).toContain('环境证明')
      }
    }
  })

  it('公开文档和引用链不应该把已安装或 CLI 可用作为环境证明替代证据', () => {
    for (const file of scannedFiles()) {
      const content = readFileSync(file, 'utf8')
  ...
  it('canonical setup-gate-rule.md 应存在且覆盖所有场景类型', () => {
    ...
    expect(content).toContain('ae:agent-browser 是唯一前置入口')
    expect(content).toContain('已有安装不能替代 proof')
    expect(content).toContain('环境证明可跨会话复用')
    expect(content).toContain('未完成 proof 校验前禁止执行')
    expect(content).toContain('环境验证失败时的降级路径')
