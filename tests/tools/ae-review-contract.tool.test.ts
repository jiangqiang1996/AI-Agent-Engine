...
describe('ae-review-contract 工具', () => {
  // test 类型契约已变更，需要 hasDesignContract 激活 test-cases-design-reviewer
  it('test 类型应返回测试文档契约并激活 test-cases-design-reviewer', async () => {
    const result = await callTool({ kind: 'test', has_design_contract: true })
  ...
  it('document 类型默认不应激活 test-cases-design-reviewer', async () => {
    const result = await callTool({ kind: 'document' })

    expect(result.kind).toBe('document')
    expect(result.documentType).toBe('general')
    expect(result.reviewers).not.toContain(AGENT.TEST_CASES_DESIGN_REVIEWER)
  })

  // design 类型契约已变更，需要 hasDesignContract 激活设计维度代理
  it('design 类型应激活 ui-ux-design-reviewer 和 test-cases-design-reviewer', async () => {
    const result = await callTool({ kind: 'design', has_design_contract: true, has_ui: true, targetTypes: 'design' })

    expect(result.documentType).toBe('design')
    expect(result.reviewers).toContain(AGENT.UI_UX_DESIGN_REVIEWER)
    expect(result.reviewers).toContain(AGENT.TEST_CASES_DESIGN_REVIEWER)
  })

  it('general 类型应返回通用文档契约且不激活 test-cases-design-reviewer', async () => {
  ...
  // hasDesignContract 激活架构设计审查者（document 域）
  it('document 类型 has_design_contract 应激活架构设计审查者', async () => {
    const result = await callTool({ kind: 'document', has_new_abstraction: true, has_design_contract: true, has_architecture_decision: true })

    expect(result.reviewers).toContain(AGENT.ARCHITECTURE_DESIGN_REVIEWER)
  })

  it('document 类型 has_product_claim 不再激活专用产品审查者（已合并至 document-reviewer）', async () => {
    const withClaim = await callTool({ kind: 'document', has_product_claim: true })
    const withoutClaim = await callTool({ kind: 'document', has_product_claim: false })

    expect(withClaim.reviewers).toContain(AGENT.DOCUMENT_REVIEWER)
    expect(withoutClaim.reviewers).toContain(AGENT.DOCUMENT_REVIEWER)
  })

  // has_config 字段语义已变更，ocr-reviewer 是 alwaysOn 始终被选中
  it('has_config 应声明为非选择字段且由 ocr-reviewer 覆盖', async () => {
    ...
    expect(result.reviewers).toContain(AGENT.OCR_REVIEWER)
  })
  ...
  it('has_ui、has_tooling 和 has_agent_config 应由 ocr-reviewer 覆盖', async () => {
    ...
    expect(uiResult.reviewers).toContain(AGENT.OCR_REVIEWER)
    expect(toolingResult.reviewers).toContain(AGENT.OCR_REVIEWER)
    expect(agentConfigResult.reviewers).toContain(AGENT.OCR_REVIEWER)
  })
  ...
  it('general 类型 targets 应激活对应审查者并返回目标覆盖', async () => {
    ...
    expect(result.reviewers).toContain(AGENT.DOCUMENT_REVIEWER)
    expect(result.reviewers).toContain(AGENT.UI_UX_DESIGN_REVIEWER)
    expect(result.reviewers).toContain(AGENT.TEST_CASES_DESIGN_REVIEWER)
    expect(result.reviewers).toContain(AGENT.OCR_REVIEWER)
    expect(result.targetCoverage?.requirements.status).toBe('covered')
    expect(result.targetCoverage?.design.status).toBe('covered')
    expect(result.targetCoverage?.asset.reviewers).toEqual([AGENT.OCR_REVIEWER])
  })
