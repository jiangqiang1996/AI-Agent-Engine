...
async function updateExisting(paths) {
...
async function freshInstall(paths) {
  console.log(`\n未检测到安装，执行全新安装流程`)

  if (existsSync(paths.repoDir)) {
    const target = paths.repoDir
    const authorized = await confirm(`目标目录已存在但不是 git 仓库: ${target}。将删除该目录并重新克隆。是否继续？`)
    if (!authorized) {
      console.log('用户取消安装。')
      rl.close()
      process.exit(0)
...
async function main() {
  ...
  if (installed) {
    await updateExisting(paths)
  } else {
    await freshInstall(paths)
  }
