...
`/ae-brainstorm` 仅做多视角发散讨论，不写文件；需要沉淀为正式需求文档时由 `/ae-prd` 承接。第一轮文档审查用于发现需求漏洞，第二轮文档审查用于检查计划可执行性，最后一轮代码审查用于检查实现风险。
...
| `/ae-brainstorm` | `[讨论主题]` | 使用多个子代理从不同视角对同一主题进行多轮发散讨论并汇总 | 不产出持久文档；需求沉淀转 `/ae-prd` |
