...
首版只交付提示、授权、归属和证据防错位能力。不自动迁移 `docs/ae/*` 产物，不自动生成或修复 worktree 路径 / 分支名冲突，不扩展 `ae:handoff` 的目标 worktree 能力。
...
- LLM 工作流：进入任何项目文件修改前，必须先完成 worktree 决策；创建 worktree 后不得在 A 会话中用 shell workdir 修改 B。
...
- 用户选择项至少包含：创建 worktree、拒绝并继续当前工作区、取消任务；自定义路径 / 分支名可通过追问完成。
- 首版不自动生成 worktree 路径和分支名；如果缺少或冲突，询问用户，不使用 `-f`、`-B` 或自动清理。
- 创建 worktree 后，当前 opencode 会话仍以 `ctx.worktree` 为准；如果用户要在 B 执行，A 会话只给出重启指引，不继续代码修改。
...
- A 会话创建 B worktree 后必须产生可复制到 B 的启动证明摘要，包含创建命令、授权证据、A 的 sessionID、A 的 `ctx.worktree`、目标 B worktree、当前分支/HEAD 和下一步指引；首版不自动写入 B，只要求用户在 B 会话中将该摘要作为 Git 操作与授权证据输入来源。
...
- `/ae-lfg` 进入实现阶段时继承 `ae:work` 的 worktree 决策，不在 `ae-lfg` 中另建一套提示逻辑；`ae-lfg` 只同步最终 gate 的新字段说明，避免双重决策分叉。
...
- [ ] 需求：覆盖 R1、R2、R3、R6、R7、R12；拒绝 worktree 允许继续，取消任务不继续实现。
- [ ] 依赖：无。
- [ ] 文件：`src/assets/skills/ae-work/SKILL.md`、`tests/assets/ae-work-worktree-text.test.ts`。
- [ ] 方法：将阶段 1“准备环境”改为通用 worktree 决策步骤；说明创建 worktree 会同步创建分支；执行 `git worktree add`、`git branch`、`git switch`、`git checkout` 前必须获得覆盖具体命令范围的授权；创建 B 后明确当前会话仍在 A；如果用户选择 B 执行，要求在 B 目录重新启动 opencode。
- [ ] 方法：拒绝 worktree 时继续询问是否创建/切换当前工作区内的功能分支；如果当前分支是默认分支且用户拒绝切分支，必须二次确认直接修改默认分支的风险，并把该选择记录到最终 Git 操作状态和 gate notes。
- [ ] 方法：当前目录不是 Git 仓库、Git 不可用或 `git worktree` 不支持时，将 worktree 决策记录为 `not_applicable`，继续强调产物归属仍以当前 `ctx.worktree` 为准。
- [ ] 执行说明：不要把首版描述成自动迁移产物；提示用户手动携带当前任务关联需求/计划，或在 B 中重新生成。
- [ ] 测试场景：技能文本包含“每次正式实现型任务”“重新启动 opencode”“不自动迁移 AE 产物”“不得在 A 会话通过 shell 工作目录修改 B”“拒绝 worktree 不等于默认分支直接实现”的语义。
...
- [ ] 需求：覆盖 R3、R4、R6、R7、R12、R13。
- [ ] 依赖：实现单元 1。
- [ ] 文件：`src/assets/skills/ae-work/SKILL.md`、`tests/assets/ae-work-worktree-text.test.ts`。
- [ ] 方法：在 `ae:work` 准备环境中要求校验 worktree 路径、分支名和 base ref；分支名使用 `git check-ref-format --branch` 或等价规则；base ref 拒绝空值、换行和 option-like 输入，并使用 `git rev-parse --verify <base>^{commit}` 或等价方式解析到最终 commit；路径必须规范化、确认不存在危险位置、符号链接绕过或 option-like 路径；执行 Git 命令必须用参数数组或等价安全执行方式，不拼接 shell 字符串；如需允许特殊路径，必须在 path 前使用 `--` 终止选项；授权前向用户展示校验后的最终参数。
- [ ] 方法：定义 A→B 启动证明摘要模板，至少包含 A 的 sessionID、A 的当前 `ctx.worktree`、目标 B worktree、当前分支、HEAD、拟执行或已执行的 Git 命令、授权来源摘要、授权覆盖范围、创建结果、B 路径、首版不自动迁移产物说明、在 B 重新启动 opencode 的指引。
- [ ] 方法：启动证明同时给出结构化字段清单或最小 JSON 片段，字段名与 gate 证据契约对齐：`source_session_id`、`operation_worktree`、`target_worktree`、`branch`、`head`、`authorization_source`、`authorization_summary`、`authorization_trust`、`authorized_at_or_message_ref`、`covered_command_args`、`final_command_args`；启动证明额外包含 `created_worktree_path`、`artifact_transfer_status`。`covered_command_args` 与 `final_command_args` 均为 `string[]` 参数数组，规范化结果由 gate-service 计算，不作为可伪造输入单独信任。
- [ ] 方法：B 中重启后的入口协议必须明确：B 已有计划文件时运行 `ae:work <B 内计划路径>`；B 没有计划文件时用户手动提供或复制需求/计划内容；B 无法获得 A 产物时重新运行 `ae:brainstorm` / `ae:plan`。A→B 启动证明不能替代 B 内真实存在的 `plan_path` / `requirements_path`。
- [ ] 执行说明：A→B 启动证明不等同于自动迁移 AE 产物；它是用户手动带到 B 的可引用上下文。B 中最终 gate 仍以 B 的 `ctx.worktree` 为边界。
- [ ] 执行说明：即使用户要求迁移产物，首版 A 会话也不得通过 `workdir=B` 写入 B 的 `docs/ae/*`；只能提供人工复制清单，或要求用户在 B 会话中重新授权后处理。未来自动迁移属于后续增强。
- [ ] 测试场景：资产文本要求校验 branch/path/base；要求 base ref 解析到 commit；要求安全执行方式；要求启动证明摘要包含 sessionID、source worktree 和 target worktree；要求用户要求迁移但无法唯一确定产物时询问用户。
...
- [ ] 方法：A 会话执行 `git worktree add` 成功后，终止状态是“执行已转移 / 等待用户在 B 重启”，不是“功能交付完成”；A 不运行最终 `ae-gate workflow:work checkpoint:final` 来宣称功能交付。
...
- 用户自定义路径 / 分支名：缺少字段、路径已存在、分支已存在或 Git 命令失败时暂停询问，不自动覆盖、不自动清理。
...
- 路径校验失败：父目录 realpath、符号链接、junction、Windows reparse point、UNC 路径、drive root 或仓库内外策略无法证明安全时，拒绝进入 Git 命令并要求用户换路径。
- base ref 异常输入：空值、换行、option-like 输入或无法解析为 commit 时拒绝进入 Git 命令；授权展示和实际执行使用解析后的最终 base 标识。
- 当前已有未提交修改：提示 worktree 不会自动带走 A 中未提交或未跟踪产物；用户仍需决定继续当前工作区、创建 B 后重新启动，或先整理当前变更。
- A 会话创建 B 后：A 只能说明下一步和记录 Git 操作，不能通过 shell `workdir=B` 修改代码。
- B 中缺少 A 的未跟踪需求 / 计划：恢复不跨 A 查找；要求用户手动复制关联产物、提供 B 内路径或重新生成。
...
- 拒绝 worktree 可以继续执行，并在最终 Git 操作状态中记录。
...
- 首版不会自动迁移 `docs/ae/*` 或自动同步 A/B 执行状态。
