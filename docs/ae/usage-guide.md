...
| `/ae-review-code` | `[mode:*] [plan:\<path\>] [base:\<ref\>]` | 使用分层角色代理和置信度门控对代码改动进行结构化审查 |
...
| `/ae-review-code-po [参数]` | 优化提示词 → `/ae-review-code <优化后>` |
...
| `/ae-review-code-pa [参数]` | 自动优化并直接执行 |
...
| 属性 | 说明 |
| --- | --- |
| 命令 | `/ae-review-code [mode:*] [plan:\<path\>] [base:\<ref\>]` |
