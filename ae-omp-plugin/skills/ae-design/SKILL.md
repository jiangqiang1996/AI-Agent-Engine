---
name: ae-design
description: "设计阶段：创建新设计或修改已有设计，产出可还原的设计文档（架构、接口、数据模型、UI/UX、测试用例、实现单元、安全、可观测性、非功能），供实施和审查对齐。触发词：设计文档、设计契约、架构设计、接口设计、数据模型、测试用例设计、实现单元拆解。适用于已有需求（ae-prd 产物或裸描述）需要转化为设计契约的场景；不适用于需求澄清（用 ae-prd）、代码实施（用 ae-work）或正式审查（用 ae-review）。"
---
...
`ae-design` 统一处理创建新设计和修改已有设计：当输入参数指向已有设计文档时，读取该设计作为基础并全量重新生成；当输入参数为 prd 文档或新描述时，创建新设计。定版是人为约定，无需技术标记——用户通过选择 `ae-design`（修改已有）或重新运行 `ae-design`（创建新设计）表达当前状态。
...
8. **总览层按方向拆分为独立文件** - 全局维度由对应的维度子代理（通用 omp task 子代理）产出独立文件（architecture.md / security.md / observability.md / non-functional.md / design-spec.md），主代理产出 overview.md + constraints.md + cross-mapping.md。模块维度由对应的维度子代理产出独立维度文件（modules/<NN>-<m>/ 下各维度文件）。采用两阶段调度：阶段 1 全局维度并行，阶段 2 模块并行。
9. **子代理产出各维度独立文件** - 不同维度的设计契约由对应的维度子代理产出独立维度文件，而非章节片段合并到单文件。
10. **深度追问设计决策** - 在产出契约前，推荐按 ae-prd 技能的「深度追问模式」逐个追问设计决策，一问一答推进直到达成共识。用户可选择跳过。
...
12. **技术实现路线约束（硬约束）** - 设计阶段必须明确技术实现路线，覆盖前端、后端、数据层、基础设施等各时段：
    ...
    - **来源优先级**：若 prd 文档中用户已明确指定技术栈，设计必须遵循该约束，不得擅自更换；若 prd 未指定，优先沿用核心原则第 20 条"存量项目感知"识别到的已有技术栈；未感知到已有技术栈时通过深度追问模式追问确定。所有推断依据必须记录在 ADR 中。
...
本技能的输入为用户调用文本：可以是需求文档路径（`ae/prds/...`）、已有设计目录路径（`ae/designs/...`）或裸描述，并可附带命名参数 `dimensions=<维度列表>`、`refactor=true`。

**如果输入为空，询问用户：** "您想设计什么？请描述您正在考虑的功能、系统或改进，或提供需求文档路径。"
...
全局维度由对应的维度子代理产出独立文件，模块维度由对应的维度子代理产出独立维度文件，主代理不直接产出维度内容（overview.md + constraints.md + cross-mapping.md 除外）。

**维度子代理不注册为 omp 代理。** 每个维度子代理 = 派一个**通用 omp task 子代理**，其 prompt 由「`references/` 下对应维度模板全文 + 需求上下文」构建，并用 outputSchema 收结构化产物摘要（见阶段 4.2）。

| 维度 | 模板（prompt 构建依据） | 产出位置 |
|------|--------|---------|
| overview / 实施约束 / 跨维度映射表 | 主代理产出（`overview-template.md`、`design-output-template.md`、`cross-dimension-mapping.md`） | `overview.md` + `constraints.md` + `cross-mapping.md` |
| design-spec | `design-spec-template.md`（mode=spec） | `design-spec.md`（含设计读数、三旋钮取值、设计体系选择、风格变体推荐、负向设计空间；同时作为 ui-ux 子代理 mode=contract 的输入） |
...
| api | `api-template.md` | `modules/<NN>-<m>/api.md` |
| database | `database-template.md` | `modules/<NN>-<m>/database.md` |
| ui-ux | `ui-ux-template.md`（mode=contract） | `modules/<NN>-<m>/ui-ux.md` + `modules/<NN>-<m>/pages/*.md` |
| test-cases | `test-cases-template.md` | `modules/<NN>-<m>/test-cases.md` |
...
**硬性约束：主代理严禁直接产出维度契约内容。** overview.md、constraints.md 和 cross-mapping.md 由主代理产出，其他维度必须派对应维度子代理产出独立文件。违反此约束属于执行错误。
...
**创建新设计 vs 修改已有设计**：当输入参数指向已有 design 路径时，读取该设计作为基础并全量重新生成；当输入参数为 prd 文档或新描述时，创建新设计。
...
2. **design** - 用户提供 `ae/designs/<name>-YYYY-MM-DD/overview.md` 路径时，作为修改已有设计的输入。读取 overview.md 和 architecture.md，作为全量重新生成的基础。
...
维度清单确认后、产出契约之前，**推荐进入深度追问模式逐个追问设计决策**（协议与 ae-prd 技能的「深度追问模式」一致：沿决策树逐个追问、每问必荐、代码库优先、一问一答推进直到共识）。使用 omp 的 `ask` 工具向用户确认是否追问：
...
沿决策树逐个追问，一问一答推进直到达成共识。追问结束后，将共识清单作为各维度子代理产出的输入。

#### 2.2 追问维度覆盖

深度追问的范围必须覆盖所有已确认维度的关键设计决策：
...
> design-spec 不需要深度追问。design-spec 子代理（mode=spec）有自己的设计决策推断流程。

#### 2.3 追问结果带回

深度追问结束后，将共识清单带回本流程，作为各维度子代理产出的输入。追问结果包含：
- 共识清单（每个关键决策的结论和理由）
- 决策依赖图（各决策之间的依赖关系摘要）
- 遗留风险（共识中仍存在的风险或不确定性）

如果用户在追问阶段选择跳过某些追问，记录跳过原因，相关维度子代理按默认推荐产出。
...
- 设计决策记录（ADR，记录关键设计决策和理由，使用稳定 ID `ADR-XXX`，从深度追问结果提炼）
...
按确认的维度清单和调度策略（全局维度并行 + 模块并行），为每个维度派一个通用 omp task 子代理产出设计契约独立文件。

#### 4.1 三步调度策略（全局维度并行 + 模块并行 + 自动校验）

采用三步调度（步骤 4.1a 全局维度并行 + 步骤 4.1b 模块并行 + 步骤 4.1c 自动校验），全程无中间大文件：

**步骤 4.1a：全局维度并行（每全局维度 1 个子代理，全并行）**

全局维度子代理并行产出独立文件：
- architecture 子代理 → `architecture.md`
- security 子代理 → `security.md`
- observability 子代理 → `observability.md`
- non-functional 子代理 → `non-functional.md`
- design-spec 子代理（mode=spec）→ `design-spec.md`

**步骤 4.1b：模块并行（每模块 1 个子代理，模块内串行产出各维度独立文件）**

按 architecture.md 中的模块划分，每个模块分配一个子代理，所有模块子代理并行执行。每个模块子代理内部串行产出各维度独立文件：
1. api 契约 → `modules/<NN>-<m>/api.md`
2. database 契约 → `modules/<NN>-<m>/database.md`（依赖 api.md 数据模型）
3. ui-ux 契约（mode=contract，需先有 design-spec.md）→ `modules/<NN>-<m>/ui-ux.md` + `modules/<NN>-<m>/pages/*.md`（依赖 api.md 响应字段）
4. test-cases 契约 → `modules/<NN>-<m>/test-cases.md`（依赖 api.md + database.md + ui-ux.md）

模块子代理按模块内命中的维度逐个执行「读模板 → 产出契约 → 记录映射表行项」循环；模块内维度间有依赖，必须串行。
...
**子代理构建方式（所有维度统一）：** 每个维度子代理都是一个通用 omp task 子代理，派发时：
- **prompt** = 对应维度模板全文（`references/<维度>-template.md`）+ 以下需求上下文：
  - prd 内容摘要
  - 深度追问结果（共识清单）
  - overview.md 上下文（稳定 ID 体系；模块维度另加 architecture.md + design-spec.md 上下文、该模块的实体清单、跨模块引用目标——ID 引用，不加载其他模块的文件）
  - 产出位置（该子代理必须写入的目标文件路径）
- **outputSchema** = 结构化产物摘要（冻结）：
...
```json
{
  ...
  "properties": {
    ...
    "coreElements": { "type": "array", "items": { "type": "string" }, "description": "已完成的核心契约元素" },
    "optionalElements": { "type": "array", "items": { "type": "string" }, "description": "已完成的可选契约元素" },
    "stableIds": { "type": "array", "items": { "type": "string" }, "description": "稳定 ID 列表（ADR/EP/T/TC/ST/COMP/PAGE）" },
    "mappingRows": { "type": "array", "items": { "type": "object" }, "description": "跨维度映射表行项" },
    "crossDimensionDeps": { "type": "array", "items": { "type": "string" }, "description": "跨维度依赖关系" },
    "lineCount": { "type": "number", "description": "行数统计" }
  },
  "required": ["files", "coreElements", "stableIds"]
}
```

**步骤 4.1a 调度（全局维度并行）：** 通过单个 `tasks[]` 批量一次性派出所有全局维度子代理（同一轮，禁止分批）。子代理产出独立文件后按 outputSchema 返回摘要。

**步骤 4.1b 调度（模块并行，模块内串行）：** 从 architecture.md 读取模块划分，为每个模块创建一个子代理。所有模块子代理在同一个 `tasks[]` 批量并行启动，每个子代理内部串行产出该模块的各维度独立文件，并按 outputSchema 返回摘要（files 含全部维度文件，mappingRows 汇总该模块全部映射行项）。

#### 4.3 主代理汇总

**所有子代理返回后**，主代理统一汇总：
- 更新 overview.md 的产物清单
- 更新 cross-mapping.md 对应行项（来自各子代理返回的 mappingRows）
- 记录稳定 ID 列表
...
14. **design-spec ↔ ui-ux** - `design-spec.md` 和 `ui-ux.md` 由同一 UI/UX 设计 persona 的两个子代理（mode=spec / mode=contract）产出，design-spec.md 的设计决策包作为 ui-ux 子代理的强制输入，内部一致性由该输入链保证。`design-spec.md` 独占设计读数（三旋钮、设计体系、风格变体、负向设计空间），`ui-ux.md` 不得出现设计读数和负向设计空间
15. **技术栈选型 ↔ 各章节契约** - 技术实现路线约束中确定的技术栈选型必须与各章节契约一致

发现不一致时，在此阶段修复后再进入 review 闭环。

**页面字段核对（涉及 UI 时）：** ui-ux.md 与 pages/ 产出后，按 `references/prototype-discipline.md` 的逐页字段核对纪律，将每个页面文件的组件放置、字段到组件映射与 prd 原型页面文件的字段清单逐项核对，不一致项修复后才进入 review 闭环。
...
调用方式：
```
/skill:ae-review mode=headless <design-dir>/overview.md
```

审查者：`design-integrity-reviewer` persona（激活条件：hasDesignContract=true）
...
| 审查结论 | 推荐下一步技能 | 引导语 |
|---------|---------------|--------|
| 通过 + 复杂任务 | **ae-work** | "设计契约已就绪。任务较复杂，建议使用 `/skill:ae-work` 按多单元编排执行。" |
| 通过 + 简单任务 | **ae-work** | "设计契约已就绪。任务较简单，可直接使用 `/skill:ae-work` 执行。" |
| 阻断 | **ae-design**（继续） | "设计契约可还原性不达标，已修复/待补充，请继续完善。" |

后续如需修改设计：再次运行 `/skill:ae-design`，传入已有设计路径作为基础。
...
- **不扩展需求边界（硬约束）** - 设计范围以用户明确提及的需求为限，无论输入是 prd 文档还是裸描述。prd 冻结后越界项回退 prd 决策。确有必要但用户未提及的设计项，须先征得用户同意方可纳入，不得自行加入。
- **主代理不直接产出维度契约** - 除 overview.md、constraints.md 和 cross-mapping.md 外，维度契约必须由对应维度子代理产出
