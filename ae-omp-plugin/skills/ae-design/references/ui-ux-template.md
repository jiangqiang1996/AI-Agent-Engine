...
**产出方：** 通用 omp task 子代理（UI/UX 设计 persona，prompt 按本模板构建）
...
## 维度子代理执行清单（mode=contract）

> 本清单由 AE 统一 UI/UX 设计代理 mode=contract 工作流合并而来。维度子代理不注册为 omp 代理：派通用 omp task 子代理，prompt = 本模板全文 + 需求上下文 + design-spec.md 设计决策包，outputSchema 收结构化产物摘要。设计读数推断与 design-spec.md 产出规范见 `design-spec-template.md`（mode=spec）。

### 输入上下文

- **需求描述 / prd 内容摘要**：功能描述、目标受众、成功标准、时段标注
- **深度追问结果**：已确认的 UI/UX 相关设计决策
- **overview 上下文**：范围映射、跨维度依赖关系、稳定 ID 体系（ST-XXX、COMP-XXX、PAGE-XXX、INT-XXX 用于本维度）
- **设计决策包**：由 `design-spec.md` 提供（设计读数、三旋钮取值、设计体系选择、风格变体推荐、负向设计空间、排版建议、色彩建议和布局关键约束）
...
1. **读取 design-spec.md**：获取设计决策包参数。不得自行重新推断设计读数或覆盖旋钮值。无上游 design-spec.md 时（如独立运行），先按 `design-spec-template.md` mode=spec 流程内部产出设计决策包再继续。
2. **读取模板和上下文**：从 design-spec.md 读取设计读数、三旋钮取值和负向设计空间作为参考，但不写入 ui-ux.md 契约（设计读数由 design-spec.md 独占）。结合 prd 需求和深度追问结果，确定本维度需要产出的契约元素。
3. **扫描项目已有组件资产**：通过 grep/glob 扫描项目组件目录（如 `src/components/`、`src/shared/`、`src/ui/` 或等效路径），识别可复用的已有组件。记录组件名、路径/位置、复用方式（直接复用 / 适配后复用）和备注（含 Props 签名摘要、功能描述），与"已有组件资产扫描"表格列对齐。未扫描到组件资产时标注"无已有组件资产，全部新建"。
4. **产出契约**：按模板产出 `modules/<NN>-<m>/ui-ux.md` + `modules/<NN>-<m>/pages/*.md`（每页独立文件）。
5. **更新跨维度映射表行项**（返回给主代理）：
   - `api-error-to-ui-state-mapping`：UI 状态机 ST-XXX 对应的 API 错误码
   - `ui-component-to-api-endpoint-mapping`：UI 组件调用的 API 端点 EP-XXX
6. **返回产出摘要**：产出独立文件路径、契约元素完成情况（核心/可选）、稳定 ID 列表（ST-XXX/COMP-XXX/PAGE-XXX）、跨维度映射表行项、组件复用统计（已有复用数 / 技术栈库引入数 / 新建自研数）、行数统计。
