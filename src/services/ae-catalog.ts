...
const PHASE_ONE_ENTRIES: AeAssetEntry[] = [
  ...
  },
  {
    ...
    description: '默认入口：编排构思、探索、计划、实施、审查等通用任务生命周期',
    argumentHint: '[任务描述|已有产物路径]',
    defaultEntry: true,
    skillFile: `src/assets/skills/${skillDir(SKILL.LFG)}/SKILL.md`,
  },
  {
    skillName: SKILL.AGENT_BROWSER,
    skillSlug: skillDir(SKILL.AGENT_BROWSER),
    commandName: COMMAND.AGENT_BROWSER,
    description: 'agent-browser 浏览器能力中枢：安装验证、环境证明、目标选择和浏览器控制',
    argumentHint: '[目标页面|操作目标|排查场景]',
    defaultEntry: false,
    skillFile: `src/assets/skills/${skillDir(SKILL.AGENT_BROWSER)}/SKILL.md`,
    customTemplate: [
      `先使用 \`${SKILL.AGENT_BROWSER}\` 技能完成 agent-browser 环境证明检查或当轮环境验证；`,
      '未完成环境证明或当轮验证前不得执行任何 agent-browser 浏览器控制命令。',
      `环境就绪后，再使用 \`${SKILL.AGENT_BROWSER}\` 技能处理这次请求，并沿用参数：\`$ARGUMENTS\`。`,
    ].join(''),
  },
  {
    ...
    description: `通过 ${SKILL.AGENT_BROWSER} 完成环境验证和目标选择后，执行浏览器端到端验收`,
    argumentHint: '[URL|路由]',
    defaultEntry: false,
  ...
  },
  {
    ...
    skillSlug: skillDir(SKILL.HTML_BUNDLE),
    commandName: COMMAND.HTML_BUNDLE,
    description: '将显式入口 HTML 及其本地静态资源收敛为自包含 bundle.html',
    argumentHint: '[entry:<HTML_PATH>] [output:<HTML_PATH>] [external:keep|fail]',
    defaultEntry: false,
  ...
  },
  {
    ...
    argumentHint: '[target:<PATH>] [mode:auto|full|incremental] [depth:shallow] [include:<PATH>...] [exclude:<PATH>...]',
    defaultEntry: false,
