...
const PHASE_ONE_ENTRIES: AeAssetEntry[] = [
  ...
  },
  {
    ...
    description: '单元测试：生成、执行、覆盖率分析，默认后端，按需支持前端组件逻辑测试，技术栈路由 Vitest/JUnit/pytest/Go test/Rust test，前端追加 Vitest/Jest + Testing Library',
    argumentHint: '[代码文件/目录] [设计用例路径(可选)] [scope=backend(默认)|frontend|all(可选)]',
  ...
  },
  {
    ...
    description: '通过 ae-ocr 工具调用 OpenCodeReview CLI 的 delegate 委托模式获取审查规格（文件清单 + 规则），不调用 LLM，审查由宿主代理执行',
    argumentHint: '[subcommand=preview|rule] [from=main] [to=branch] [paths=...]',
...
const REQUIRED_AGENTS: ReadonlyArray<readonly [string, AgentDefinition['stage'], string, string?]> = [
  ...
  [AGENT.OCR_REVIEWER, 'review', 'OCR 代码审查主引擎：通过 ae-ocr delegate 模式获取审查规格（文件清单 + 规则），由本代理执行审查，覆盖 bug/安全/性能/可维护性/测试覆盖/风格/规范/对抗式/代理就绪/可靠性'],
  ...
  [AGENT.UNIT_TEST_RUNNER, 'test', '单元测试执行代理：生成、执行单元测试并分析覆盖率，默认后端，按需支持前端组件逻辑测试（Vitest/Jest + Testing Library），支持 Vitest/JUnit/pytest/Go test/Rust test'],
