...
export function loadGraphConfig(worktree: string, builtinConfigFile = createRuntimeAssetManifest(import.meta.url).builtinConfigFile): GraphConfig {
  ...
  for (const layer of graphConfigLayers(paths)) {
    ...
    if (Array.isArray(graph.include)) {
      include.push(...graph.include.filter((item): item is string => typeof item === 'string'))
    }
    if (Array.isArray(graph.exclude)) {
      exclude.push(...graph.exclude.filter((item): item is string => typeof item === 'string'))
    }
  }
...
export function updateGraphRulesInProjectConfig(worktree: string, changes: GraphRuleChanges): GraphConfig {
  ...
  if (
    ...
    && normalized.removeInclude.length === 0
    && normalized.removeExclude.length === 0
  ) {
    return loadGraphConfig(worktree)
  }
  ...
  nextRaw = updateArrayProperty(
    ...
    normalized.removeExclude,
  )
  writeFileSync(configPath, nextRaw.endsWith('\n') ? nextRaw : `${nextRaw}\n`)
  return loadGraphConfig(worktree)
}

export function saveGraphIncludeRule(worktree: string, rule: string): GraphConfig {
  return updateGraphRulesInProjectConfig(worktree, { appendInclude: [rule] })
}

export function saveGraphExcludeRule(worktree: string, rule: string): GraphConfig {
  return updateGraphRulesInProjectConfig(worktree, { appendExclude: [rule] })
}
