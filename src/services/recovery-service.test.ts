...
describe('recovery-service', () => {
  ...
  it('应该在存在多个候选时要求用户选择', () => {
    ...
    writeFileSync(join(root, 'docs', 'ae', 'plans', 'a.md'), '---\ntitle: A\ntype: feat\nstatus: active\ndate: 2026-04-18\noriginFingerprint: one\n---\nA')
    writeFileSync(join(root, 'docs', 'ae', 'plans', 'b.md'), '---\ntitle: B\ntype: feat\nstatus: active\ndate: 2026-04-18\noriginFingerprint: two\n---\nB')
  ...
  it('应该在上游指纹不匹配时阻断恢复', () => {
    ...
    mkdirSync(join(root, 'docs', 'ae', 'plans'), { recursive: true })
    writeFileSync(join(root, 'docs', 'ae', 'plans', 'plan.md'), '---\ntitle: Plan\ntype: feat\nstatus: active\ndate: 2026-04-18\noriginFingerprint: old\n---\nPlan')
  ...
  it('应该在高优先级旧产物失配时回退到匹配的上游计划', () => {
    ...
    mkdirSync(join(root, 'docs', 'ae', 'plans'), { recursive: true })
    writeFileSync(join(root, 'docs', 'ae', 'work', 'run.md'), '---\nstatus: completed\noriginFingerprint: old\ntype: work\n---\nwork')
  ...
  it('应该忽略上游指纹不匹配的损坏 work 产物并回退到匹配计划', () => {
    ...
    writeFileSync(
      join(root, 'docs', 'ae', 'plans', 'plan.md'),
      '---\ntitle: Plan\ntype: feat\nstatus: active\ndate: 2026-04-18\noriginFingerprint: new\n---\nplan',
    )
