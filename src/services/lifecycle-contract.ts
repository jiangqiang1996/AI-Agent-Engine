...
const LIFECYCLE_CATALOG_DESCRIPTIONS: Record<LifecycleCatalogStageId, string> = {
  prd: '探索阶段：创建新需求或修改已有需求，澄清目标、边界、约束、成功标准和待定问题，并产出需求文档',
  design: '设计阶段：创建新设计或修改已有设计，澄清设计决策并产出设计文档，含概览、架构、接口、数据模型、测试用例与验收标准，供实施和审查对齐。支持 dimensions 参数指定部分维度，支持 refactor=true 彻底重构',
