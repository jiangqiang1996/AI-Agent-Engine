...
export interface BrowserDiscoveryResult {
  ...
  pageUrlHash: string
  targetNodeId?: string
  targetBinding: 'target_export' | 'unbound'
...
function parseDiscoveryOutput(stdout: string): { pageUrl: string; targetNodeId?: string; targetBinding: 'target_export' | 'unbound'; resourceUrls: string[] } {
  let parsed: unknown
  try {
    parsed = JSON.parse(stdout) as unknown
  } catch {
    throw new FigmaAssetError('agent-browser eval 输出无法解析，请尝试 mode: collect 手动导出。', 'agent_browser_parse_failed')
  }
  if (!isDiscoveryOutput(parsed)) {
    return { pageUrl: '', targetBinding: 'unbound', resourceUrls: [] }
  }
  return {
    pageUrl: parsed.pageUrl,
    targetNodeId: typeof parsed.targetNodeId === 'string' ? parsed.targetNodeId : undefined,
    targetBinding: parsed.targetBinding === 'target_export' ? 'target_export' : 'unbound',
    resourceUrls: parsed.resourceUrls.filter((item): item is string => typeof item === 'string'),
  }
}

function isDiscoveryOutput(value: unknown): value is { pageUrl: string; targetNodeId?: unknown; targetBinding?: unknown; resourceUrls: unknown[] } {
