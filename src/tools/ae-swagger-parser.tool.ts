...
export const aeSwaggerParserTool = tool({
  description: [
    '解析 Swagger/OpenAPI JSON 并生成接口联调摘要。',
    '',
    '功能说明：',
    '- 支持工作区内 JSON 文件和远程 HTTP(S) URL',
    '- 支持 Swagger 2.0 和 OpenAPI 3.x 常见结构',
  ...
  args: {
    source: z.string().min(1).describe('Swagger/OpenAPI JSON 来源，可以是工作区内本地路径或 HTTP(S) URL。'),
    ...
    try {
      if (/^https?:\/\//i.test(args.source)) {
        await Effect.runPromise(ctx.ask({
          ...
          always: [],
          metadata: {
            action: '读取远程 Swagger/OpenAPI JSON',
