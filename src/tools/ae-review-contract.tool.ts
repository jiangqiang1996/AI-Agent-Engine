...
export const aeReviewContractTool: ToolDefinition = tool({
  description: [
    ...
    '',
    '功能说明：',
    '- 根据审查类型与模式生成 reviewer team',
    '- 返回 gate 语义和模式边界',
    '',
    '适用场景：',
    '- markdown skill 需要先确定 reviewer team 再并行派发时',
    '- README 或测试需要校验公开审查契约时',
    '',
    '不适用场景：',
    '- 不负责真正执行 reviewer 子代理',
    '- 不负责写入 findings 或审查产物',
  ].join('\n'),
  args: {
        ...
        return JSON.stringify(
          {
            ...
            mode: args.mode,
            reviewers,
            gate: args.kind === 'code' ? 'P0/P1 默认阻断；report-only 只报告' : '文档与计划审查默认作为 gate 使用',
          },
          null,
          2,
        )
