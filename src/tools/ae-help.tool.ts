...
export const aeHelpTool = tool({
  description: [
    ...
    '- 开发专精代理（@frontend-dev、@backend-dev、@backend-fix、@frontend-fix，由 ae:work 编排层通过 Task 调度）',
    '- 自定义命令',
    '',
    '适用场景：',
    '- 用户输入 /ae-help 命令',
    ...
    '',
    '详情查询：',
    '- 传入完整元素名（如 ae:design、/ae-design、@ocr-reviewer）可查看该元素的详细说明和关联信息',
  ...
  args: {
    query: z
      .string()
      .optional()
      .describe('过滤关键词或精确元素名。传入完整元素名（如 ae:design、/ae-design、@ocr-reviewer）返回详情视图；传入关键词返回子串匹配的列表。为空时展示全部。'),
  },
