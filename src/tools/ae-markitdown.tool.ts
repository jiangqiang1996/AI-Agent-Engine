...
export const aeMarkitdownTool = tool({
  description: [
    ...
    '',
    '功能说明：',
    '- 支持格式：HTML、CSV（含 TSV）、JSON、DOCX、XLSX、PDF、PPTX、JPG（含 PNG）',
    ...
    '- 如需保留多次转换历史（例如源文件已修改），可在用户明确再次要求时显式调用，工具不会去重',
    '',
    '适用场景：',
    '- 用户明确要求将本地文档持久化为 Markdown 文件（用于归档、分发或后续处理）',
    ...
    '- 文本类格式（HTML/CSV/TSV/JSON）仅当用户明确要求转换为 Markdown 文件时使用；用户只需查看/理解内容时，应直接使用 Read 工具读取，禁止调用本工具',
    '- 图片格式（JPG/PNG）仅当满足以下条件之一时使用：当前模型不支持读取图片文件，或用户明确要求将图片内容转换为 Markdown 文件；模型支持读取图片且只需查看/理解图片内容时，应直接使用 Read 工具，禁止调用本工具',
  ].join('\n'),
  args: {
    file: z
      .string()
