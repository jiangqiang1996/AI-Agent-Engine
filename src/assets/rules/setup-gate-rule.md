# agent-browser 环境门禁规则

**本规则为全局硬约束，适用于任何会话、任何角色需要执行浏览器操作的场景。**

无论通过技能、代理、命令、工具、bash、MCP 还是通过 prompt optimize 生成的新会话，只要需要执行 `agent-browser` 命令，就必须先确认当前工作区存在合法的 agent-browser 环境证明；证明缺失或无效时必须先完成 `ae:agent-browser` / `/ae-agent-browser` 环境验证流程。无例外。
...
2. **已有安装不能替代 proof** — `agent-browser` 已安装、`command -v` / `Get-Command` / `where` 检查成功、用户声称已安装或本地 CLI 可用性检查通过，都不能替代 `ae-agent-browser-proof` 的机器校验结果。
...
1. 在流程开始处加入 `ae-agent-browser-proof action=check` 前置检查
2. 未完成环境证明时，不得继续到可复制命令或 bash 执行步骤
3. 环境验证失败时提供降级路径，不得跳过环境门禁直接执行
4. 如为新会话生成提示词（prompt optimize 场景），提示词中必须包含 proof 检查和 `ae:agent-browser` 兜底要求
...
- 证明文件缺失、损坏或结构不合法时，等同于未完成 agent-browser 环境验证
