...
无论通过技能、代理、命令、工具、bash、MCP 还是通过 prompt optimize 生成的新会话，只要意图执行 `agent-browser` 命令，就必须先确认当前工作区存在合法的 agent-browser 环境证明；证明缺失或无效时必须先完成 `ae:agent-browser` / `/ae-agent-browser` 环境验证流程。无例外。
...
2. 未完成环境证明时，不得继续到可复制命令区或 bash 执行区
