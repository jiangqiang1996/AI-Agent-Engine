...
Windows 下原生模块 DLL（`node_modules/@napi-rs/canvas` 的 `skia.win32-x64-msvc.node`）一旦被进程映射就无法删除，而 `/ae-uninstall` 本身在 AE 插件进程内执行，因此**退出码 2 是 Windows 上的常见正常结果，不是失败**。
...
如需彻底清理：关闭所有 opencode 进程后，重新执行同一条卸载命令。

验证方式：重启后尝试 /ae-help，该命令不再可用即表示卸载已生效。
```

残留路径取自脚本 stderr 输出的"以下 N 项被占用，未能删除"清单，原样列出，不得自行编造或省略。
