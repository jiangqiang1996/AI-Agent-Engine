...
脚本用退出码区分三种结果，**必须逐个范围读取退出码，禁止无条件报告卸载成功**：

| 退出码 | 含义 | 该范围结论 |
|-------|------|-----------|
| 0 | 已彻底删除 | 卸载完成 |
| 2 | `ae-server.js` 已删除（插件已注销），但有被进程占用的残留文件 | 卸载已生效，残留待清理 |
| 1 | `ae-server.js` 未能删除，插件仍会被加载 | **卸载未生效** |
| 其他非零 | 脚本异常中断 | **卸载未生效** |

Windows 下原生模块 DLL（`node_modules/@napi-rs/canvas-win32-x64-msvc` 的 `skia.win32-x64-msvc.node`）一旦被进程映射就无法删除，而 `/ae-uninstall` 本身在 AE 插件进程内执行，因此**退出码 2 是 Windows 上的常见正常结果，不是失败**。

退出码 2 且 assets 目录存在残留时，脚本会**保留仓库目录**（`<target-dir>/ai-agent-engine-src`，内含卸载脚本自身），并在 stdout 打印一条可直接复制执行的重跑命令（形如 `node <卸载脚本绝对路径> --target-dir <目标目录> --yes`）。这是有意设计：若此时删除仓库，恢复路径会连同卸载脚本一起消失。报告时必须原样转述该命令，不得改写为"重新执行本命令"这类无具体入口的表述。
...
插件入口已删除、插件已注销，但需如实说明残留：

```
AE 插件已注销（全局/项目级）——插件入口 plugins/ae-server.js 已删除，重启 opencode 后不再加载。

以下文件被当前进程占用，未能删除（仅占磁盘，不影响卸载效果）：
  <脚本 stderr 输出的残留路径>

仓库目录已保留（内含卸载脚本）。如需彻底清理：关闭所有 opencode 进程后重新执行：
  <脚本 stdout 输出的完整重跑命令>

验证方式：重启后尝试 /ae-help，该命令不再可用即表示卸载已生效。
```

残留路径取自脚本 stderr 输出的"以下 N 项被占用，未能删除"清单，重跑命令取自脚本 stdout 输出的 `node ... --target-dir ... --yes` 一行，两者均原样列出，不得自行编造、省略或改写。

### 存在退出码 1 或其他非零的范围

必须明确告知卸载未生效，禁止报告"卸载完成"：

```
AE 插件卸载未生效（全局/项目级）——插件入口 plugins/ae-server.js 删除失败，opencode 仍会加载 AE 插件。

失败原因：<脚本 stderr 输出>

处理方式：关闭所有 opencode 进程后重新执行卸载命令；仍失败则手动删除
  <target-dir>/plugins/ae-server.js
  <target-dir>/plugins/ai-agent-engine/
```
