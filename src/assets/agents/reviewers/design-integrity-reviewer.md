...
`ae/designs/` 下存在 2 个及以上维度产物时激活。当只有单一维度产物时无需本代理。

## Workflow

1. **独立读取全部设计维度文件**（不依赖其他代理输出）：读取 `index.md` 获取模块导航，读取 `global.md` 的全局章节（§系统架构、§安全、§可观测性、§非功能、§设计规范），读取各 `modules/<m>.md` 的模块章节（§API、§Database、§UI/UX、§Test Cases）。
