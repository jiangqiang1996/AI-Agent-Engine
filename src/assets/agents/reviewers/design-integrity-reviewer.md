...
1. **独立读取全部设计维度文件**（不依赖其他代理输出）：读取 `index.md` 获取模块导航，读取 `global.md` 的全局章节（§系统架构、§安全、§可观测性、§非功能、§设计规范），读取各 `modules/<NN>-<m>/` 目录下的维度文件（`api.md`、`database.md`、`ui-ux.md`、`test-cases.md`）。
