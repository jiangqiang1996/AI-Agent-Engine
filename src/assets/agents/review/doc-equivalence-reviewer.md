...
3. 建立转换后文档语义清单。
4. 对比两份清单，找出遗漏、新增、语义漂移、ID 丢失、模板不兼容和自包含性问题；“下一步”或后续流程指令属于工作流路由信息，不纳入语义等价清单，也不得仅因其存在、缺失或由模板补回而报告等价性问题。
...
7. 当审查 A → B → C 往返转换时，比较 A 与 C 的正文结构；除 frontmatter、标题文本、转换产物溯源元数据和“下一步”工作流路由章节外，C 的正文必须恢复 A 的规范章节顺序、稳定 ID、需求/实现单元字段和等价性计数。
...
- 不审查“下一步”或后续流程指令的等价性；`ae:doc-humanize` 可删除该章节，`ae:doc-structure` 可按目标模板补回该章节。
- 不允许根据常识补充源文档没有的信息。
