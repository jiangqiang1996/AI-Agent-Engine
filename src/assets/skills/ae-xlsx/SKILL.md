---
name: ae:xlsx
description: "创建、编辑、分析 Excel 电子表格（.xlsx），全面覆盖 exceljs 能力：完整单元格样式、合并、冻结、筛选、条件格式、数据验证、工作簿属性。通过 ae-xlsx 工具实现，输出仍为 .xlsx。"
argument-hint: "[创建|编辑|分析|公式] [文件路径] [任务描述]"
---

# ae:xlsx — Excel 电子表格处理

创建、编辑、分析 `.xlsx` 文件，全面覆盖 exceljs 能力，支持完整单元格样式、公式、合并单元格、冻结窗格、自动筛选、条件格式、数据验证、工作表属性和工作簿属性。通过内置 `ae-xlsx` 工具实现，无需安装额外依赖。

## 与 ae:markitdown 的边界

| 场景 | 用 ae:markitdown | 用 ae:xlsx |
|------|------------------|------------|
| 只读提取表格数据供 LLM 阅读 | ✅ 优先用 markitdown | ❌ |
| 创建新 Excel | ❌ | ✅ 输出 .xlsx |
| 编辑现有 Excel（保留公式格式） | ❌ | ✅ 输出 .xlsx |
| 数据分析和结构信息 | ❌ | ✅ 返回行列数和预览 |
| 需要完整样式、合并、冻结等高级功能 | ❌ | ✅ 全面支持 |

**原则：只需读取数据时用 `ae:markitdown`；需要创建或修改 .xlsx 文件时用本技能。**
