...
创建新电子表格或大规模修改已有电子表格时，应先调度 `@doc-architect` 代理制定风格规格书和内容结构，再按规格书执行。

### 何时调度 @doc-architect

| 场景 | 是否调度 |
|------|----------|
| 创建全新电子表格（3+ 工作表或复杂样式需求） | ✅ 先调度 @doc-architect 制定风格规格 |
| 大规模修改已有电子表格（2+ 工作表需要样式调整） | ✅ 先调度 @doc-architect 重新规划风格 |
| 小范围单元格编辑 | ❌ 直接用 edit |
| 追加行数据 | ❌ 直接用 add-rows |
| 添加单个新工作表 | ❌ 直接用 add-sheet |

### 协作流程

1. 调度 `@doc-architect`，传入文档类型（XLSX）、目标受众和风格偏好
2. @doc-architect 输出风格规格书和内容结构
