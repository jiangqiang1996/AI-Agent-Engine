...
argument-hint: "<技能名或需求描述> [--global] [--no-command|--command-only]"
---

# OpenCode 原生技能创建与更新器

使用本技能帮助用户创建或更新项目级、全局级 OpenCode 技能和命令。支持三种创建模式：只创建技能、只创建命令、同时创建技能和命令。推荐同时创建，并用命令包装同名技能；如果用户没有明确选择创建模式，必须先询问，不要静默采用默认值。只有用户明确要求“全局”“全局级”或传入 `--global` 时，才处理全局级技能和命令。
...
9. 编辑 `SKILL.md`：写清角色、适用场景、输入处理、执行步骤、边界和验证方式；更新时优先保留仍有效的现有约束。
10. 编辑命令文件：包装技能的命令正文应明确要求加载并使用同名技能，并传递 `$ARGUMENTS`；独立命令正文必须直接包含完整处理流程，并保留 `$ARGUMENTS`。
