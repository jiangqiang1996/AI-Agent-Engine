...
**触发条件：** 必产出
**产出文件：** `test-cases/` 子目录下多个文件（索引 + 按功能域×测试层二维分组）
...
1. **每端点 ≥ 7 场景** — 1 正常 + ≥2 等价类有效代表 + 2 边界值 + ≥1 错误码触发 + 1 幂等验证（如适用）。无幂等时需 ≥2 错误码触发，确保总数 ≥ 7。错误码用例同时满足第 6 条约束，覆盖密度统计时按端点归属计数，错误码覆盖在第 6 条独立统计
...
6. **每错误码 ≥ 1 触发用例** — api 维度定义的每个错误码必须被至少 1 个用例触发
...
| 需求 | 正常场景 | 边界场景 | 异常场景 | 测试设计方法 | 测试层级 | 维度契约元素 |
|------|---------|---------|---------|------------|---------|------------|
| 需求 1 | TC-001 | TC-005 | TC-BE-002 | EP+BVA+DT | 后端 | api:EP-001, database:T-resources |
| 需求 2 | TC-FE-001 | TC-FE-004 | TC-FE-006 | ST+EG | 前端 | ui-ux:ST-form, api:EP-001 |

> 测试设计方法列标注该需求主要使用的方法：EP=等价类、BVA=边界值、DT=决策表、ST=状态转换、PW=成对组合、EG=错误猜测

### 覆盖密度统计

| 维度契约元素 | 应有场景数 | 实际用例数 | 覆盖率 | 缺口 |
|------------|----------|----------|-------|------|
...
> 覆盖密度统计必须在索引层产出，列出每个端点/状态机/错误码/数据库约束/权限角色的应有场景数（按量化标准）与实际用例数对比，覆盖率 < 95% 的项必须在缺口列说明。状态机应有场景数 = 合法边数 + 关键非法边数（非 N×N 全部边）
...
| ST-button | success→idle（合法，重置） | TC-UI-003 | 完整 |
| ST-button | error→idle（合法，重试） | TC-UI-004 | 完整 |
| ST-button | idle→success（非法，跳过loading） | TC-UI-005 | 完整（关键非法边） |
| ST-form | idle→loading（合法） | TC-FE-003 | 完整 |
| ST-form | loading→success（合法） | TC-FE-003 | 完整 |
| ST-form | loading→error（合法） | TC-FE-004 | 完整 |
| ST-form | error→idle（合法，重试） | TC-FE-005 | 完整 |

> 每个状态机的每条转换边（N×N 矩阵中的合法边逐条 + 关键非法边）必须单独列出一行，而非只列路径。合法边 = 状态机定义中允许的转换；关键非法边 = 不应到达的状态转换（如 idle→success 跳过 loading）
...
> 覆盖清单列出所有按测试设计方法枚举的场景，每条含预分配的稳定 ID（TC-XXX）、场景描述、方法来源（EP/BVA/DT/ST/PW/EG）和对应契约元素 ID。file-plan 根据覆盖清单按功能域×测试层分组。
...
当索引文件超 300 行时（大型项目覆盖清单可能有数百个场景行），将覆盖密度统计表和完整性表外迁为独立文件：
- `test-cases/01-test-cases.md`：保留覆盖矩阵、覆盖清单、file-plan、负向设计空间（核心索引内容）
- `test-cases/01b-coverage-stats.md`：覆盖密度统计表 + 接口覆盖完整性表 + 交互覆盖完整性表 + 状态机覆盖完整性表 + 验收映射 + 维度覆盖追溯

外迁后 `01-test-cases.md` 引用 `01b-coverage-stats.md`，Split Manifest 中标注 layer: index-auxiliary。
...
### 阶段 2：分组实体层（按 功能域 × 测试层 分组，串行生成 + 即时校验）

file-plan 按"功能域 × 测试层"二维分组，而非仅按测试层分 4 文件。大型项目典型产出 15-40 个 test-cases 文件。

分组规则：
1. 功能域为第一分组键，测试层为第二分组键
2. 单文件 ≤ 300 行，超限按子域或场景类型再拆
3. 同功能域的后端 + 前端 + 集成用例分别产出独立文件，保持高内聚

文件命名格式：`NN-<test-layer>-<domain>.md`（NN 为序号，从 02 开始）。
示例：`02-backend-auth.md`、`03-frontend-auth.md`、`04-integration-auth.md`、`05-backend-resource.md`...

#### frontend-<domain>.md（按功能域分组的前端用例，≤ 300 行）

文件名格式：`NN-frontend-<domain>.md`（NN 为序号，从 02 开始）。
...
#### backend-<domain>.md（按功能域分组的后端用例，≤ 300 行）

文件名格式：`NN-backend-<domain>.md`（NN 为序号）。
...
#### integration-<domain>.md（按功能域分组的集成 + 契约测试，≤ 300 行）

文件名格式：`NN-integration-<domain>.md`（NN 为序号）。
...
#### non-functional-<domain>.md（按功能域分组的性能 + 安全 + 架构 + 可观测性测试，≤ 300 行）

文件名格式：`NN-non-functional-<domain>.md`（NN 为序号）。
...
- `[核心]` **覆盖矩阵表**：需求 × 场景 × 边界 × 测试设计方法 × 测试层级 × 维度契约元素的交叉覆盖表
- `[核心]` **覆盖密度统计表**：每个端点/状态机/错误码/数据库约束的应有场景数 vs 实际用例数 vs 覆盖率 vs 缺口
- `[核心]` **接口覆盖完整性表**：每个 api 端点 → 正常/异常/边界/等价类/决策表用例 → 覆盖状态
- `[核心]` **交互覆盖完整性表**：每个 ui-ux 交互行为 → 用例 → 覆盖状态
- `[核心]` **状态机覆盖完整性表**：每个状态机转换边（N×N 矩阵）→ 用例 → 覆盖状态
- `[核心]` **P0-P3 用例表**：每条含稳定 ID `TC-XXX`、场景、前置、步骤、预期、断言、维度契约追溯、测试设计方法来源
