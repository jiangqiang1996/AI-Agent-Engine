...
创建新 PDF 或大规模修改已有 PDF 时，应先调度 `@doc-architect` 代理制定风格规格书和内容大纲，再按规格书执行。

### 何时调度 @doc-architect

| 场景 | 是否调度 |
|------|----------|
| 创建全新 PDF（5+ 页） | ✅ 先调度 @doc-architect 制定风格规格 |
| 大规模修改已有 PDF（3+ 页需要风格调整） | ✅ 先调度 @doc-architect 重新规划风格 |
| 小范围局部更新（1 页叠加元素） | ❌ 直接用 update-page |
| 单页追加 | ❌ 直接用 add-pages |
| 填写表单/添加水印/旋转页面 | ❌ 直接用对应操作 |

### 协作流程

1. 调度 `@doc-architect`，传入文档类型（PDF）、目标受众和风格偏好
2. @doc-architect 输出风格规格书和内容大纲
