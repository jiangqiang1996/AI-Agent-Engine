...
6. **验收完成门禁（硬约束）：** ae:web-forge 必须完成其执行阶段 3（@browser-inspector 浏览器验收）后，本技能方可视为交付完成。若 ae:web-forge 报告"验收阻断"（MCP 未就绪导致浏览器验收无法执行），本技能不得声明任务完成或交付产出文件，必须向用户明确报告验收阻断原因，并提示用户启动浏览器或重新执行 `ae:html-slides` 以完成验收。不得将未验收的产出文件作为交付凭证。
