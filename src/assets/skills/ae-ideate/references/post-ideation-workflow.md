...
使用一致的评分标准对入选想法评分：落地程度、预期价值、新颖性、务实性、杠杆、实施负担、与更强想法的重叠。

目标输出：
- 默认保留 5-7 个入选想法
- 如果入选想法过多，运行第二轮更严格的筛选
- 如果少于 5 个入选想法，如实报告而非降低标准

## 阶段 4：呈现入选想法

**检查点 B。** 写入 `<scratch-dir>/survivors.md`，包含入选想法列表加上关键上下文。尽力写入。

以结构化形式仅呈现入选想法：标题、描述、理由、缺点、置信度分数、预估复杂度。
