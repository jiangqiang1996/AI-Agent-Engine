---
name: ae:pptx
description: "创建、编辑、分析 PowerPoint 演示文稿（.pptx），支持元素化绘制、母版、章节、图表、媒体等全面能力。通过 ae-pptx 工具实现，输出仍为 .pptx。"
argument-hint: "[创建|编辑|分析|模板] [文件路径] [任务描述]"
---

# ae:pptx — PowerPoint 演示文稿处理

创建、编辑、分析 `.pptx` 文件。通过内置 `ae-pptx` 工具实现，无需安装额外依赖。底层使用 pptxgenjs，全面覆盖其 API 能力。
