...
创建新演示文稿或大规模修改已有演示文稿时，应先调度 `@doc-architect` 代理制定风格规格书和内容大纲，再按规格书执行。

### 何时调度 @doc-architect

| 场景 | 是否调度 |
|------|----------|
| 创建全新演示文稿（5+ 张幻灯片） | ✅ 先调度 @doc-architect 制定风格规格 |
| 大规模修改已有演示文稿（3+ 张幻灯片需要风格调整） | ✅ 先调度 @doc-architect 重新规划风格 |
| 小范围文本替换（1-2 张幻灯片） | ❌ 直接用 edit/update-slide |
| 追加风格一致的幻灯片 | ❌ 沿用已有风格，无需规格书 |
| 单张幻灯片局部更新 | ❌ 直接用 update-slide |

### 协作流程

1. 调度 `@doc-architect`，传入文档类型（PPTX）、目标受众和风格偏好
