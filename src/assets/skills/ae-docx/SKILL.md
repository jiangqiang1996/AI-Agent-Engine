...
创建新文档或大规模修改已有文档时，应先调度 `@doc-architect` 代理制定风格规格书和内容大纲，再按规格书执行。

### 何时调度 @doc-architect

| 场景 | 是否调度 |
|------|----------|
| 创建全新文档（5+ 个内容块） | ✅ 先调度 @doc-architect 制定风格规格 |
| 大规模修改已有文档（3+ 个内容块需要风格调整） | ✅ 先调度 @doc-architect 重新规划风格 |
| 小范围文本替换 | ❌ 直接用 edit |
| 追加风格一致的内容块 | ❌ 沿用已有风格，无需规格书 |
| 单个内容块更新 | ❌ 直接用 update-block |

### 协作流程

1. 调度 `@doc-architect`，传入文档类型（DOCX）、目标受众和风格偏好
...
- **create** 操作：生成文件自动写入 `ae/documents/docx/` 子目录，文件名规则：`<名称>-<操作>-<时间戳>-<随机串>.docx`。如需自定义路径，传入 `outputPath` 参数
