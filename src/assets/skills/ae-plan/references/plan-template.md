...
- `format: human-readable-plan` 表示计划同时面向人读和机器提取；设计由计划承载时，不需要额外生成独立设计文件。
- 仅当模块数量大于 1，或用户明确要求分片时，才创建分片计划或设计文档；不得按功能数量、实现单元数量、文档行数或预估 token 数自动分片。
- 分片主文件使用 `sharded: true` 和 `shards` 索引，但仍必须保留来源目标、全局范围、跨模块流程、共享数据、接口边界、关键决策和依赖关系，不能退化为分片路径列表。
- 分片计划子文件使用 `type: plan-shard`、`parent: <主文件仓库相对路径>` 和 `module: <模块名>`；分片设计子文件使用 `type: design-shard`、`parent` 和 `module`。子文件只作为提取和审查输入，不作为恢复、门禁或 `ae:work` 执行入口的顶层产物。
