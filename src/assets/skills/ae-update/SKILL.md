...
执行任何 Git 写操作前，必须确认目标目录是 AE 插件安装或源码维护仓库，并取得用户对目标仓库、目标分支、工作区、完整命令参数、授权来源和具体命令 `git reset --hard HEAD`、`git clean -fd --exclude=node_modules`、`git pull` 的明确授权。未授权时必须停止更新流程。必须提示这些命令会丢弃该安装仓库中的本地未提交修改和未追踪文件。
