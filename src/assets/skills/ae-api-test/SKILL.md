...
检查是否提供了设计用例路径：
- **有设计用例** → 从用例规格编译接口测试骨架：提取测试场景、接口调用顺序、输入参数、期望输出
...
1. 根据接口数量选择输出模式（单文件内联 / 模块化）
2. 从模板库中选取所需的请求方法
3. 组合：基础配置 + 环境安全护栏 + 认证片段 + 流程化请求序列 + 数据传递 + 断言 + 运行器 + JSON report
4. 生成业务流程测试脚本
...
脚本写入后按**先流程后边界**的顺序自动运行测试并修复问题（分层归因修复，最多 3 轮）。
