...
- `mode`：可选，`browser`、`api`、`collect` 或 `validate`，默认 `api`；`browser` 仍处于实验能力验证阶段，需显式指定。
...
| 条件 | 推荐模式 | 说明 |
|------|---------|------|
| 有 Figma URL + allowlist token 来源 | `api` | 默认路径；调用官方 images API 下载节点素材 |
| 有 Figma URL + 当前会话已完成 `ae:setup` | `browser` | 实验路径；需显式指定 `mode: browser`，用户在隔离浏览器中登录，工具内部 runner 尝试发现页面资源并受控下载 |
...
browser 自动下载仍处于实验能力验证阶段；未显式指定 `mode: browser` 时不要选择该路径。若工具提示 browser 能力暂不可用，降级到 `api` 或 `collect`。
...
失败降级优先建议 `mode: collect` 手动导出；`api` 作为用户明确提供 token 时的默认下载路径。
...
- MVP 默认采用 API token 路径下载明确节点素材；browser 授权资源发现 + 受控写盘保留为实验路径。
