...
计划文档和 worktree 交接输入默认调用 `ae-task-analyzer` 工具。裸提示词默认使用输入分流阶段的轻量任务分解结果，仅在文件范围无法判断、任务边界明显跨模块、需要并行安全矩阵，或轻量分解不足以生成待办时调用工具。

工具不可用、返回错误，或任务已判定为简单内联时，允许手动分析并记录降级原因。

调用规则：

- worktree 交接输入：按 B worktree 续执行路径提取可选计划路径；有计划路径且文件存在时使用 `mode:plan`，无计划、计划路径不存在，或工具无法从计划提取单元时，必须从交接文件的 `## A→B Startup Proof`、`## Execution Baseline` 和验证要求手动构建待办。
- 计划文档输入：使用 `mode:plan`，传入计划路径。
- 裸提示词输入：优先使用轻量任务分解；需要升级时才使用 `mode:scan`，传入任务描述。

工具输出结构化任务列表 `units`、文件冲突矩阵 `conflict_matrix` 和并行组 `parallel_groups`。如果工具返回警告（如无法识别文件）或 `units` 为空，降级为手动任务拆分；不得因为任务分析工具未识别计划格式而停止续执行。

## 待办构建

从计划推导任务，包含依赖关系、文件范围、共享产物、验证要求和优先级排序；`ae-task-analyzer.units` 可直接作为 `todo_units`。
