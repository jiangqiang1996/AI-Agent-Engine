...
| 任务特征 | 路由代理 | 说明 |
|---------|---------|------|
| 修复已有页面的视觉/交互/接口问题 | `@web-fix` | 统一前端修复入口，具备诊断→修复→验证内部闭环 |
| 浏览器 E2E 测试、验收测试、测试生成 | `@e2e-tester` | 浏览器测试代理，覆盖 plan/generate/heal 全链路 |
...
如果未匹配到任何专精代理，服务层会兜底选中 debug-fix；`specialistCount` 为 0 仅在工具异常时出现，此时报错并提示用户检查参数。
