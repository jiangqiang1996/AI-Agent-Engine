...
- A 会话创建 B worktree 后，不得继续实现；必须迁移当前任务已确定、真实存在的需求/设计产物、`ae/graphs/` 和 `.opencode/ae.jsonc`（A 端条件必选：上游产物或物理文件存在时必须迁移，不存在时才不传），plan_path 为无条件必选（无上游 ae:plan 产物时 A 端须生成上下文派生计划并迁移），并调用 `ae-worktree-handoff` 工具生成交接文件；存在性判断和复制必须使用文件系统视角，即使路径被 `.gitignore` 忽略也必须按真实文件系统存在性迁移，不能依赖 `git status`、`git ls-files` 或其他会受 `.gitignore` 影响的 Git 视角；未迁移的产物不得出现在交接文件中，禁止自行拼接交接 Markdown。
