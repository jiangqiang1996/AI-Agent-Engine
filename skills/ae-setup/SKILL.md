...
使用 Bash 工具执行以下命令检查 agent-browser 是否已安装：

command -v agent-browser 2>/dev/null || where agent-browser 2>NUL

- 如果输出包含 `agent-browser` 的路径：已安装，进入第四步
- 如果输出为空或报错：未安装，进入第二步
...
⚠️ agent-browser 未安装。agent-browser 是 AE 前端设计技能的核心依赖，用于浏览器截图、自动化测试和设计迭代。
...
- 验证成功：报告安装成功
- 验证失败：提示用户手动安装，并给出安装命令和备选方案（npx）
